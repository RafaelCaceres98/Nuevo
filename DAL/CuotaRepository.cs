...
namespace DAL
{
    public class CuotaRepository
    {
        ...
        public int NumeroCuotasPagadas( int Codigocredito) {
            using (var command = Connection._conexion.CreateCommand())
            {
                command.CommandText = "NumeroCuotasPagadas";
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.Add("Codigocredito", OracleDbType.Int32).Value = Codigocredito;
                command.Parameters.Add("registro", OracleDbType.RefCursor).Direction = ParameterDirection.Output;
                return int.Parse(((object)command.ExecuteScalar()).ToString());
            }
