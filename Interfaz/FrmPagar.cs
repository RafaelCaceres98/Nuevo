...
namespace Interfaz
{
    public partial class FrmPagar : Form
    {
        ...
        private void DtgCreditos_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            TxtCodigoCredito.Text = (((int)DtgCreditos.Rows[e.RowIndex].Cells[2].Value)).ToString();
        }
        ...
        public void PintarTabla() {
            DtgCreditos.Rows.Clear();
            foreach (var item in creditoservice.ConsultarCreditosCliente(cliente.Cedula))
            {
                DtgCreditos.Rows.Add(cliente.PrimerNombre, cliente.Cedula, item.NumeroCredito, item.ValorCredito, item.TasaInteres, item.NumeroCuotas, item.ValorCuota, cuotaservice.NumeroCuotasPagadas(item.NumeroCredito));
